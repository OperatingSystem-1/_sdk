...
function isExcluded(relPath, excludeSet) {
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        if (excludeSet.has(partial) || ALWAYS_EXCLUDE.has(partial))
            return true;
    }
    if (EXCLUDE_EXTS.has(extname(relPath).toLowerCase()))
        return true;
    return false;
}
...
async function walkFiles(dir, base, excludeSet) {
    const results = [];
    let entries;
    try {
    ...
    if (existsSync(scriptsDir)) {
        try {
            ...
            for (const se of scriptEntries) {
                ...
                if (se.name === 'tq' || se.name.endsWith('.service'))
                    continue;
                if (se.name === '__pycache__')
                    continue;
...
// ─── Package ────────────────────────────────────────────────────────────────
export async function packageAgentState(opts) {
...
export async function packageExplicitFiles(opts) {
    const files = [];
    const report = {
        identityFiles: [],
        memoryFiles: 0,
    ...
    for (const [bundlePath, absPath] of Object.entries(opts.files)) {
        ...
        if (bundlePath.startsWith('identity/'))
            report.identityFiles.push(basename(bundlePath));
        else if (bundlePath.startsWith('memory/'))
            report.memoryFiles++;
        else if (bundlePath.startsWith('skills/'))
            report.skillCount++;
        else if (bundlePath.startsWith('scripts/'))
            report.scriptCount++;
        else
            report.workspaceFiles++;
    }
