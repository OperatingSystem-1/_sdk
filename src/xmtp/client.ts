...
const clientCache = new Map<string, Promise<XmtpClient>>();
...
function ensureSigningKey(config: ClientConfig): `0x${string}` {
...
function getDbEncryptionKey(): `0x${string}` {
  mkdirSync(XMTP_DIR, { recursive: true, mode: 0o700 });
  if (existsSync(DB_KEY_FILE)) {
    return readFileSync(DB_KEY_FILE, 'utf-8').trim() as `0x${string}`;
  }
...
export function getXmtpIdentifier(address: string): Identifier {
  return {
...
export function getXmtpAddress(config: ClientConfig): string {
  const account = privateKeyToAccount(ensureSigningKey(config));
  return account.address.toLowerCase();
}

export async function getXmtpClient(config: ClientConfig): Promise<XmtpClient> {
  const privateKey = ensureSigningKey(config);
  const account = privateKeyToAccount(privateKey);
  const address = account.address.toLowerCase();
  const env = (process.env.MI_XMTP_ENV ?? 'production') as 'production' | 'dev' | 'local';
...
export async function getGroupConversation(config: ClientConfig, conversationId: string) {
    ...
    for (let attempt = 0; attempt < 4 && !conversation; attempt++) {
      ...
      try {
        await client.preferences.setConsentStates([{
          entityType: CONSENT_ENTITY_GROUP_ID as any,
          state: CONSENT_STATE_ALLOWED as any,
...
export async function getDmConversation(config: ClientConfig, peerAddress: string) {
  const client = await getXmtpClient(config);
